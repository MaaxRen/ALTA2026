#!/usr/bin/env python3
"""LoRA fine-tune Qwen3-Embedding-0.6B with two shared-encoder task heads."""
...
try:
    ...
    raise SystemExit(
        "Qwen LoRA training requires PEFT. Install the project training dependencies first."
    ) from exc
...
MODEL_NAME = str(
    REPO_ROOT / "resources" / "pretrained_model" / "qwen3-embedding-0.6b"
)
LOCAL_FILES_ONLY = True
DATA_DIR = REPO_ROOT / "data" / "besstie"
OUTPUT_DIR = REPO_ROOT / "model_checkpoints" / "qwen_embedding_0.6b_multitask_lora"
TRAINING_SUMMARY_DIR = (
    REPO_ROOT / "training_summary" / "runs" / "qwen_embedding_0.6b_multitask_lora"
)
...
def main() -> None:
    ...
    for split_seed in SPLIT_SEEDS:
        print(f"\n=== Qwen LoRA split seed {split_seed} ===")
        ...
        all_results[str(split_seed)] = run_training(
            config, tokenizer, model_factory, method="lora_all_linear"
        )
        gc.collect()
    ...
    print("\n=== Qwen LoRA results across splits ===")
